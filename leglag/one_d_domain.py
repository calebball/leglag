...
class OneDDomain():
    ...
    def eri_array(self):
        ...
        try:
            ...
            if self.functions > 0:
                self._eri_array = [self.build_eri_array(domain_two)
                    if domain_two.functions > 0 else None
                    for domain_two in self.molecule.domains]
            else:
                self._eri_array = [None
                    for domain_two in self.molecule.domains]
            return self._eri_array
    ...
    def double_bar_array(self):
        ...
        in another domain of the parent molecule."""
        return [eris - np.swapaxes(eris, 1, 3)
                if i == self.molecule.domains.index(self) else eris
    ...
    def mo_eri_array(self):
        ...
        try:
            if self._mo_eri_array_out_of_date:
                if self.functions > 0:
                    self._mo_eri_array = [
                        rollaxis(dot(rollaxis(dot(rollaxis(dot(rollaxis(dot(
                            self.eri_array[i],
                            domain_two.orbital_coefficients), 3),
                            domain_two.orbital_coefficients), 3),
                            self.orbital_coefficients), 3),
                            self.orbital_coefficients), 3)
                        if domain_two.functions > 0 else None
                        for i, domain_two in enumerate(self.molecule.domains)]

                else:
                    self._mo_eri_array = [None 
                        for domain_two in self.molecule.domains]
    ...
    def weizsaecker(self, x):
        ...
        return ((bfp.dot(self.density_matrix.dot(bf)) +
            bf.dot(self.density_matrix.dot(bfp)))**2 /
            (2 * self.rho(x)))

    def tau(self, x):
        """Returns the kinetic energy density at a point."""
        bfp = self.bf_derivative_vector(x)
        return 2 * sum(bfp.dot(self.orbital_coefficients[:,k])**2
            for k in range(self.electrons))

    def eta(self, x, return_rho=False):
        """Returns the electronic curvature parameter at a point."""
        if self.electrons <= 1: return (0, 0) if return_rho else 0
        bf = self.bf_vector(x)
        rho = bf.dot(self.density_matrix.dot(bf))
        if rho < 1e-100: return (rho, np.inf) if return_rho else np.inf
        bfp = self.bf_derivative_vector(x)

        tau = 2 * sum(bfp.dot(self.orbital_coefficients[:,k])**2
            for k in range(self.electrons))

        weizsaecker = ((bfp.dot(self.density_matrix.dot(bf)) +
            bf.dot(self.density_matrix.dot(bfp)))**2 /
            (2 * rho))
        ...
        if tau - weizsaecker < 0:
            ...
            return (rho, (normalisation * (tau - weizsaecker))
                    if return_rho else normalisation * (tau - weiasaecker))
    ...
    def scf_cycle(self, p_array, fns=None):
        ...
        self.diis_fock.append(
            self.kinetic_matrix + self.potential_matrix +
            sum(dot(x[0].reshape(-1, len(x[1])), x[1])
                if type(x[0]) == np.ndarray and type(x[1]) == np.ndarray else 0
                for x in zip(self.double_bar_array, p_array)
                ).reshape(self.functions, self.functions))
        self.diis_error.append(
            dot(self.diis_fock[-1][:fns,:fns],
                self.density_matrix[:fns,:fns]) -
            dot(self.density_matrix[:fns,:fns],
                self.diis_fock[-1][:fns,:fns]))

        # Solve the DIIS equation
        diis_mat = np.fromiter((np.trace(dot(i, j))
                             for i in self.diis_error
                             for j in self.diis_error), np.double)
        diis_mat.resize(len(self.diis_error), len(self.diis_error))
        diis_mat = np.vstack((diis_mat, -np.ones(len(self.diis_error), np.double)))
        diis_mat = np.hstack((diis_mat, (
            (arange(len(self.diis_error) + 1) //
             len(self.diis_error)) - 1)[:,None]))
        try:
            diis_vec = solve(diis_mat,
                -(arange(len(self.diis_error) + 1) // len(self.diis_error)))
        ...
        w, v = eigh(sum(x[0] * x[1][:fns,:fns]
            for x in zip(diis_vec, self.diis_fock)))
        ordering = w.argsort()
        self.orbital_energies = np.pad(w[ordering], (0, self.functions - fns),
...
class InfDomain(OneDDomain):
    ...
    def build_eri_array(self, domain_two):
        ...
        if isinstance(domain_two, FinDomain):
            ...
            if not check:
                ...
                tmp = sorted(val for d in self.molecule.domains
                        for val in d.quad_level.values() if val != 0)
                median = tmp[len(tmp) // 2] if len(tmp) > 5 else 0
                start = max(domain_two.quad_level[self],
                            self.molecule.quadrature_start,
                            median)

            result, quad = leglag.two_e_integrals.true_inf_fin(self, domain_two,
                quad_check=check, quad_start=start)
            self.quad_level[domain_two] = quad

            return result
        elif isinstance(domain_two, InfDomain):
            return (leglag.two_e_integrals.true_inf_inf(self, domain_two)
                if self != domain_two else leglag.two_e_integrals.quasi_inf_inf(self))
    ...
    def bf_vector(self, x):
        ...
        return np.fromiter((eval_genlaguerre(mu - 1, 2, 2 * a * z) * 
            c / np.sqrt(mu * (mu + 1))
            for mu in range(1, self.functions + 1)), dtype=np.double)
    ...
    def bf_derivative_vector(self, x):
        ...
        z = abs(self.position - x)
        if self.side:
            return np.fromiter((np.sqrt(8 * a**3 / (mu * (mu + 1))) *
                np.exp(- a * z) *
...
class FinDomain(OneDDomain):
    ...
    def __init__(self, num, A, B, electrons, functions, molecule):
        ...
        super().__init__(num, (max(A, B), min(A, B)), electrons, functions,
            molecule)
        self.halfwidth = abs(B - A) / 2

        self.s_star = np.zeros((functions, functions), dtype=np.double)
        with open(resource_filename('leglag',
                'integral_data/PP_bound.dat')) as bound_file:
            for line in bound_file:
                (m, n, value) = line.split()
                m = int(m) - 1
                n = int(n) - 1
        ...
        self.s_star_p = np.zeros((functions, functions), dtype=np.double)
        with open(resource_filename('leglag',
                'integral_data/PP_p_bound.dat')) as bound_file:
            for line in bound_file:
                (m, n, value) = line.split()
                m = int(m) - 1
                n = int(n) - 1
        ...
        with open(resource_filename('leglag',
                'integral_data/PP_n_bound.dat')) as bound_file:
            for line in bound_file:
                (m, n, value) = line.split()
                m = int(m) - 1
                n = int(n) - 1
    ...
    def build_eri_array(self, domain_two):
        ...
        if isinstance(domain_two, FinDomain):
            if self == domain_two:
                ...
                result, quad = leglag.two_e_integrals.true_fin_fin(self, domain_two,
                    quad_check=check, quad_start=start)
                self.quad_level[domain_two] = quad
            ...
            result, quad = leglag.two_e_integrals.true_inf_fin(domain_two, self,
                quad_check=check, quad_start=start)
            self.quad_level[domain_two] = quad
    ...
    def ele_com(self, i):
        ...
        return (self.position[1] + self.halfwidth +
...
def build_potential_matrix(domain):
    pass
build_potential_matrix.register(InfDomain,
        leglag.one_e_integrals.inf_nuclear_attraction)
build_potential_matrix.register(FinDomain,
        leglag.one_e_integrals.fin_nuclear_attraction)

@singledispatch
def build_field_matrix(domain):
    pass
build_field_matrix.register(InfDomain,
        leglag.one_e_integrals.inf_field)
build_field_matrix.register(FinDomain,
        leglag.one_e_integrals.fin_field)
